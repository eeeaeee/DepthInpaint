...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='./train/datasets',)
    parser.add_argument('--data_path_test', type=str, default='./test/datasets')
    parser.add_argument('--data_name', type=str, default='inpaint', choices=['rain100L', 'rain100H', 'inpaint'])
    parser.add_argument('--save_path', type=str, default='test_places_on_celebahq')
    ...
    parser.add_argument('--num_iter', type=int, default=100003, help='iterations of training')
...
class TrainDataset(Dataset):
    def __init__(self, data_path, data_path_test, data_name, data_type, patch_size=None, length=None):
        ...
        self.corrupt_images = sorted(glob.glob('{}/input/*.png'.format(data_path)))
        self.clear_images = sorted(glob.glob('{}/target/*.png'.format(data_path)))

        self.corrupt_images_test = sorted(glob.glob('{}/input/*.png'.format(data_path_test)))
    ...
    def __getitem__(self, idx):
        ...
        if self.data_type == 'train':
            corrupt_path = self.corrupt_images[idx % self.num]
            gt_path = self.clear_images[idx % self.num]
        else:
            corrupt_path = self.corrupt_images_test[idx % self.num_test]
            gt_path = self.clear_images_test[idx % self.num_test]
        
        corrupt_name = os.path.basename(corrupt_path)
        gt_name = os.path.basename(gt_path)
        name_wo_ext = os.path.splitext(corrupt_name)[0]

        corrupt_rgb_pil = Image.open(corrupt_path)
        depth_dir = './your/depth_img'
        depth_path = os.path.join(depth_dir, f'{name_wo_ext}_depth.png')
        depth_pil = Image.open(depth_path).resize(corrupt_rgb_pil.size, Image.BILINEAR)

        corrupt_rgb = T.to_tensor(corrupt_rgb_pil)
        depth = T.to_tensor(depth_pil)
        clear = T.to_tensor(Image.open(gt_path))

        corrupt = torch.cat([corrupt_rgb, depth], dim=0)
        ...
        h, w = corrupt.shape[1:]
        return corrupt, clear, corrupt_name, h, w
