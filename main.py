...
def save_loop(net, data_loader, num_iter):
    ...
    # save statistics
    data_frame = pd.DataFrame(data=results, index=range(1, (num_iter if args.model_file else num_iter // 50000) + 1))
...
if __name__ == '__main__':
    args = parse_args()
    wandb.init(
    project="all_data_baseline", settings=wandb.Settings(mode="offline"),
    name="ffhq-data_resize",
    config={
        "dataset": args.data_name,
        "learning_rate": args.lr,
        "num_iter": args.num_iter,
        "batch_size": args.batch_size,
        "model": "Blind_Omni_Wav_Net",
        "transformer_blocks": args.num_blocks,
...
for n_iter in train_bar:
    ...
    wandb.log({
                ...
                "l1_loss": F.l1_loss(out, norain).item(),
                "ssim_loss": ssim(out, norain).item(),
                "perc_loss": perceptual_loss(out, norain).item(),
    ...
    if n_iter % 50000 == 0:
        wandb.log({
                    "output_image": [wandb.Image(out[0].clamp(0, 1).cpu(), caption="Output")],
                    "target_image": [wandb.Image(norain[0].clamp(0, 1).cpu(), caption="Target")],
                    "input_image": [wandb.Image(rain[0].cpu(), caption="Corrupted")]
               })
